...
def test_chopper(tmdb_id, title, year, media_type="movie", season=1, episode=1, imdb_id=""):
    ...
    if media_type == "movie":
        url = f"https://api.videasy.net/mb-flix/sources-with-title?title={enc_title}&mediaType={media_type}&year={year}&tmdbId={tmdb_id}&imdbId={imdb_id}"
    else:
        url = f"https://api.videasy.net/mb-flix/sources-with-title?title={enc_title}&mediaType={media_type}&year={year}&episodeId={episode}&seasonId={season}&tmdbId={tmdb_id}&imdbId={imdb_id}"
