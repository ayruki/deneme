import requests

def test_vegapunk_46668():
    tmdb_api_key = "a2f888b27315e62e471b2d587048f32e"
    api_key = "9iQNC5HQwPlaFuJDkhncJ5XTJ8feGXOJatAA"
    signature = ""
    
    headers = {
        "signature": signature,
        "hash256": "f4d4bc98a3fc4600e7f2c2bab7533f1f03d8a70ff03c256bb11dc57050536bd0",
        "user-agent": "EasyPlex (Android 13; SM-A546E; samsung; tr)"
    }
    
    id = 46668
    
    # 1. Get info from TMDB
    tmdb_url = f"https://api.themoviedb.org/3/tv/{id}?api_key={tmdb_api_key}&language=en-US"
    tmdb_resp = requests.get(tmdb_url).json()
    title = tmdb_resp.get("name") or tmdb_resp.get("original_name")
    print(f"[*] TMDB Title: {title}")
    
    # 2. Search Vegapunk
    import urllib.parse
    encoded_title = urllib.parse.quote(title)
    search_url = f"https://ydfvfdizipanel.ru/public/api/search/{encoded_title}/{api_key}"
    search_resp = requests.get(search_url, headers=headers).json()
    
    search_results = search_resp.get("search", [])
    candidates = [item for item in search_results if item.get("type") in ["serie", "anime"]]
    print(f"[*] Candidates found: {len(candidates)}")
    
    # 3. Verify and Fetch details
    for candidate in candidates:
        c_id = candidate.get("id")
        c_type = candidate.get("type")
        c_name = candidate.get("title") or candidate.get("name")
        
        detail_url = ""
        if c_type == "serie":
            detail_url = f"https://ydfvfdizipanel.ru/public/api/series/show/{c_id}/{api_key}"
        elif c_type == "anime":
            detail_url = f"https://ydfvfdizipanel.ru/public/api/animes/show/{c_id}/{api_key}"
            
        print(f"\n[*] Fetching details for '{c_name}' (ID: {c_id}, Type: {c_type})")
        detail_resp = requests.get(detail_url, headers=headers).json()
        
        tmdb_id_raw = str(detail_resp.get("tmdb_id"))
        if tmdb_id_raw == str(id):
            print(f"  -> TMDB ID Matches! ({tmdb_id_raw})")
            seasons = detail_resp.get("seasons", [])
            for season in seasons:
                season_num = season.get("season_number")
                episodes = season.get("episodes", [])
                print(f"  -> Season {season_num} has {len(episodes)} episodes.")
                
                # Check S1E1 specifically
                if season_num == 1:
                    for ep in episodes:
                        if ep.get("episode_number") == 1:
                            videos = ep.get("videos", [])
                            print(f"    -> Found S1E1: {ep.get('name')} | Videos count: {len(videos)}")
                            for v in videos:
                                print(f"      -> Link: {v.get('link')} | Server: {v.get('server')}")
        else:
            print(f"  -> TMDB ID Mismatch: {tmdb_id_raw} != {id}")

if __name__ == "__main__":
    test_vegapunk_46668()
